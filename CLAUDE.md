...
- **Showcase demo app lives under `projects/showcase/`** (`@angular/build:application`, standalone + zone.js, Angular Material 3 theme). Routes lazy-load one page per library feature from `src/app/pages/*`; the feature list is centralized in `src/app/shared/nav.ts`. It imports `@ferhaps/easy-ui-lib`, resolved to `projects/ui-lib/src/public-api.ts` by the `paths` mapping in the root `tsconfig.json` — so demos always reflect the current source. Mounts `<eui-error-handler>` and `<eui-global-loader>` once in the app shell.

Key building blocks (under `projects/ui-lib/src/lib/`):

- **Error handling pipeline** — `ErrorService` (`services/error.service.ts`) is a `providedIn: 'root'` RxJS `Subject` that broadcasts `HttpErrorResponse`s; `ErrorHandlerComponent` subscribes and opens `ErrorPopupComponent` in a `MatDialog`. `ErrorDisplayComponent` and the `SnakeCaseParserPipe` render human-readable messages. Drop `<eui-error-handler>` once near the app root.
- **Global loader** — `LoaderService` + `GlobalLoaderComponent` provide an app-wide loading overlay.
- **Confirm dialog** — `ConfirmDialogService.confirm(options)` opens the internal `ConfirmDialogComponent` (built on `DefaultDialogComponent`) and resolves a `Promise<boolean>` — `true` only on an explicit confirm; cancel/Escape/close/backdrop all resolve `false`. Supports `danger` styling for destructive actions and focuses the safe (cancel) action first.
- **`TableComponent<T>`** (`components/table/`) — a generic, config-driven table. Behavior is declared via a `Config<T>` object (columns via `dataProps`, sorting, drag-drop reordering, single/multiple row selection, row options, `classRules`, `trackBy`, `loading`, `emptyMessage`) and it emits a single typed `TableEvent<T>` `output` for all interactions. Rows are tracked by identity (override with `Config.trackBy`) and selection is keyed to that identity, so it survives drag-drop reordering and resets when a new `data` array is supplied; `TableEvent.selectedRows` reports indices into the current `data` order. Selection state and derived flags (`hasOptions`, `areAllRowsSelected`) are signal/`computed`-based; rows, sort headers and row menus are keyboard-accessible (`aria-sort`/`aria-selected`). Pairs with `TableSortHeaderComponent`.
- **Form helpers** — `PasswordValidatorDirective` (configurable rules via inputs; reports which rules failed under the `passwordInvalid` key as `PasswordErrors`) and `FieldsMatchValidatorDirective` (re-validates its field automatically when the mirrored field changes) hook into `NG_VALIDATORS`; `PhoneValidationDirective` formats input via a `ControlValueAccessor` (`NG_VALUE_ACCESSOR`), keeping the bound form control in sync; `SearchBarComponent` is a debounced search input (configurable `debounceMs`) that supports all three forms styles — it implements both `ControlValueAccessor` (reactive/template-driven) and `FormValueControl` (Signal Forms, exposing a `value` model bound via `[formField]`). It has no internal `FormGroup`; the committed value (debounced + trimmed) is reconciled into the raw input via an `effect`. `PasswordStrengthComponent` (`eui-password-strength`) renders a strength meter + per-rule checklist off the same exported `validatePasswordRules` engine as the password validator (identical rule inputs).
- **HTTP interceptor** — `easyUiLibInterceptor` (`interceptors/`) reads the `X-Global-Loader` / `X-Skip-Error` request headers to drive `LoaderService` and `ErrorService` (stripping them before send). `provideEasyUiLib(otherInterceptors?)` (`providers/`) is a convenience that registers `HttpClient` with the interceptor wired in (and any extra functional interceptors passed in, which run after it); apps needing other `provideHttpClient` features or already calling `provideHttpClient` should instead add `easyUiLibInterceptor` to their own `withInterceptors([...])`.
...
- **Theming** — component SCSS reads Angular Material M3 system variables (`--mat-sys-*`) with neutral hex fallbacks, so components adopt the consuming app's Material theme (incl. dark mode) and still look correct with no theme. When adding styles, prefer `var(--mat-sys-TOKEN, #fallback)` over raw hex.
